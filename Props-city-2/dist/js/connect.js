...
require(["config"], function () {
  ...
  require(["jquery", "migrate"], function ($) {
    //alert("www");
    var user = $("#user").val();
    var pass = $("#password").val(); //var us = $("#user");
    ...
    $("#login").on("click", function () {
      var $ur = $("#user").val();
      var $pa = $("#password").val(); //		   	     console.log($ur);
      //		   	     console.log($pa);

      if ($ur == "") {
        alert("请输入用户名"); //$("#user").val("");

        $("#password").val(""); //console.log("www")
      } else if ($pa == "") {
        alert("请输入密码"); //console.log("ddc")
      } else if (!($ur == "") && !($pa == "")) {
        var $user = $("#user").val();
        ...
        $.ajax({
          ...
          success: function success(res) {
            ...
            if (res.code == 1) {
              console.log("110"); //$("#user").val("");
              //$("#password").val("");
              //用户名存cookie

              var user = $("#user").val(); //console.log($.cookie)
